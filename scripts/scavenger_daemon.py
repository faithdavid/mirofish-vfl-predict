...
from msport_client import MSportClient
...
def _recover_state(db_path: Path) -> tuple[set, set]:
    ...
    try:
        ...
        print(f"[SCAV] 🧠 Recovered {len(predicted)} predicted, {len(settled)} settled matchdays from DB.")
    except Exception as e:
        print(f"[SCAV] ⚠️  DB recovery error: {e}")
    return predicted, settled
...
def _push(endpoint: str, payload: dict, timeout: int = 15) -> dict | None:
    ...
    try:
        ...
        if r.status_code == 200:
            return r.json()
        print(f"[SCAV] ❌ Server {endpoint} → HTTP {r.status_code}: {r.text[:200]}")
    except requests.exceptions.ConnectionError:
        pass  # Server not up yet — silently retry
    except Exception as e:
        print(f"[SCAV] ❌ Push error to {endpoint}: {e}")
    return None
    ...
    while True:
        try:
            ...
            if curr_key not in predicted_keys:
                ...
                if odds_data:
                    print(f"[SCAV] ✅ Odds for MD {curr_md} received ({len(odds_data.get('events', []))} fixtures)")
                    ...
                    if result:
                        ...
                        print(f"[SCAV] 🚀 Predicted MD {curr_md}: {result.get('inserted',0)} rows, {result.get('locks',0)} LOCKs")
                    else:
            ...
            if prev_md and prev_season:
                ...
                if prev_key not in settled_keys:
                    ...
                    if results_data:
                        ...
                        print(f"[SCAV] 📊 Results for MD {prev_md}: {len(rl)} matches")
                        ...
                        if res:
                            ...
                            print(f"[SCAV] ✅ Settled MD {prev_md}: {res.get('settled',0)} matches, P&L: {res.get('profit',0):+.2f}")
                    ...
                    if bf_key not in settled_keys:
                        ...
                        if rd:
                            ...
                            if r:
                                ...
                                print(f"[SCAV] 🔄 Backfilled MD {bf_md}: {r.get('settled',0)} settled")
            ...
            print(f"[SCAV] ⚠️  Unhandled loop error: {e}")
