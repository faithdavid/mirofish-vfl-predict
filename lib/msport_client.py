    ...
    def _load_auth(self, auth_path: Path):
        ...
        if auth_path.exists():
            try:
                ...
                print("[MSPORT] ✅ Auth loaded from msport_auth.json")
            except Exception as e:
                print(f"[MSPORT] ⚠️ Failed to load auth: {e} — using defaults")
        else:
            print("[MSPORT] ⚠️ msport_auth.json not found — using default headers")
    ...
    def _get(self, url: str, params: dict = None, timeout: int = 10) -> dict | None:
        ...
        try:
            ...
            if r.status_code == 200:
                return r.json()
            print(f"[MSPORT] ⚠️ HTTP {r.status_code} from {url}")
            return None
        except requests.exceptions.Timeout:
            print(f"[MSPORT] ⏰ Timeout hitting {url}")
            return None
        except Exception as e:
            print(f"[MSPORT] ❌ Request error: {e}")
            return None
