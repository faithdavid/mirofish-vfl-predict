...
function initNav() {
    document.querySelectorAll('.nav-link').forEach(link => {
        link.addEventListener('click', e => {
            ...
            // Map legacy IDs to new structure
            let targetView = `view-${view}`;
            if (view === 'dashboard') targetView = 'view-dashboard';
            if (view === 'history')   targetView = 'view-history';
            if (view === 'inject')    targetView = 'view-inject';
            ...
            if (view === 'dashboard') {
                ...
                loadVerification();
                historyTimer = setInterval(loadVerification, 10000);
    ...
    // Tab buttons for Manual Injection
    document.querySelectorAll('.btn-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const isOdds = target === 'odds-input';
            document.getElementById('odds-input').style.display = isOdds ? 'block' : 'none';
            document.getElementById('results-input').style.display = isOdds ? 'none' : 'block';
            document.getElementById('btn-predict').style.display = isOdds ? 'block' : 'none';
            document.getElementById('btn-audit').style.display = isOdds ? 'none' : 'block';
        });
    });

    document.getElementById('btn-refresh-raw')?.addEventListener('click', loadDataInspector);
}
...
async function loadVerification() {
    try {
        const res  = await fetch('/api/history');
        const data = await res.json();
        renderVerification(data.history || []);
    } catch (e) {
        console.error('[VERIFY] Error loading verification:', e);
    }
}

function renderVerification(history) {
    const body = document.getElementById('verify-body');
    if (!body) return;
    body.innerHTML = '';

    if (history.length === 0) {
        body.innerHTML = '<div class="radar-empty">No settled matches verified yet.</div>';
        return;
    }

    history.forEach(h => {
        const isHit  = (h.profit || 0) > 0;
        const row    = document.createElement('div');
        row.className = `radar-row ${isHit ? 'verify-hit' : 'verify-miss'}`;
        
        const resLabel = isHit ? '🎯 HIT' : (h.outcome ? '❌ MISS' : '⌛ PENDING');
        const resClass = isHit ? 'outcome-match' : (h.outcome ? 'outcome-miss' : 'outcome-neutral');

        row.innerHTML = `
            <span class="fixture-name">${h.fixture}</span>
            <span class="mono">MD ${h.matchday}</span>
            <span><span class="pred-badge pred-${h.prediction}">${predLabel(h.prediction)}</span></span>
            <span class="mono" style="color:var(--text-dim)">${h.actual || '—'}</span>
            <span class="${resClass}">${resLabel}</span>
            <span class="mono">${h.certainty}%</span>
        `;
        body.appendChild(row);
    });
}
...
async function loadDataInspector() {
    const display = document.getElementById('json-odds-display');
    if (!display) return;
    
    display.innerText = 'Loading raw payloads...';
    try {
        const res = await fetch('/api/last_raw');
        const data = await res.json();
        display.innerText = JSON.stringify(data, null, 2);
    } catch (e) {
        display.innerText = 'Failed to fetch raw feed.';
    }
}
